/* ----------------------------------------------------------------------------
 * This file was automatically generated by SWIG (http://www.swig.org).
 * Version 4.0.2
 *
 * Do not make changes to this file unless you know what you are doing--modify
 * the SWIG interface file instead.
 * ----------------------------------------------------------------------------- */

package arblib;

import static arblib.Constants.*;

public class ComplexMatrix implements AutoCloseable {
  private transient long swigCPtr;
  protected transient boolean swigCMemOwn;

  public ComplexMatrix(long cPtr, boolean cMemoryOwn) {
    swigCMemOwn = cMemoryOwn;
    swigCPtr = cPtr;
  }

  public static long getCPtr(ComplexMatrix obj) {
    return (obj == null) ? 0 : obj.swigCPtr;
  }

  public synchronized void delete() {
    if (swigCPtr != 0) {
      if (swigCMemOwn) {
        swigCMemOwn = false;
        arblibJNI.delete_ComplexMatrix(swigCPtr);
      }
      swigCPtr = 0;
    }
  }


  @Override
  public void close()
  { 
      delete();
  }
  
  public ComplexMatrix init(int rows, int cols)
  {
    arblib.acb_mat_init(this, rows, cols);
    return this;
  } 
  

  public void setEntries(Complex value) {
    arblibJNI.ComplexMatrix_entries_set(swigCPtr, this, Complex.getCPtr(value), value);
  }

  public Complex getEntries() {
    long cPtr = arblibJNI.ComplexMatrix_entries_get(swigCPtr, this);
    return (cPtr == 0) ? null : new Complex(cPtr, false);
  }

  public void setR(int value) {
    arblibJNI.ComplexMatrix_r_set(swigCPtr, this, value);
  }

  public int getR() {
    return arblibJNI.ComplexMatrix_r_get(swigCPtr, this);
  }

  public void setC(int value) {
    arblibJNI.ComplexMatrix_c_set(swigCPtr, this, value);
  }

  public int getC() {
    return arblibJNI.ComplexMatrix_c_get(swigCPtr, this);
  }

  public void setRows(SWIGTYPE_p_p_acb_struct value) {
    arblibJNI.ComplexMatrix_rows_set(swigCPtr, this, SWIGTYPE_p_p_acb_struct.getCPtr(value));
  }

  public SWIGTYPE_p_p_acb_struct getRows() {
    long cPtr = arblibJNI.ComplexMatrix_rows_get(swigCPtr, this);
    return (cPtr == 0) ? null : new SWIGTYPE_p_p_acb_struct(cPtr, false);
  }

  public ComplexMatrix() {
    this(arblibJNI.new_ComplexMatrix(), true);
  }

}
